...
├── vector_store/        ✅ COMPLETE - Supabase pgvector integration
...
**Supabase pgvector integration** with RPC-based similarity search:

- **Storage**: Cosine similarity search (pgvector)
- **Metadata**: Category, tags, user_id, session_id, created_at
- **Operations**: Similarity search with metadata filters
- **Deployment**: Supabase (managed Postgres)
...
### ✅ **Ingestion Layer (n8n)** - Production Ready
**Decoupled ingestion** with Supabase as the single source of truth:

**Current State**:
- ✅ **Google Drive** as the primary source of truth
- ✅ **Chunking + embeddings** executed in n8n
- ✅ **Supabase pgvector** stores `documents` with metadata + embeddings
- ✅ **Health check**: `GET /rag/health` reflects Supabase connectivity

**Document Payload** (n8n → Supabase):
```json
{
  "content": "Chunk text...",
  "metadata": {
    ...
    "section_title": "The Pomodoro Technique",
    "source": "google_drive",
...
- Embedding model must match backend query model
- Vector dimension must match Supabase `vector(1536)`
- Backend never ingests documents at startup
...
supabase_collection_name: str = "documents"
supabase_query_function: str = "match_documents"
...
**Internal Flow**:
1. **Retrieve**: Embed query → Search Supabase → Return top-k docs (scored)
...
4. ✅ ~~Set up n8n ingestion pipeline~~ - Google Drive → Supabase pgvector
5. ✅ ~~Define Supabase schema + RPC~~ - `match_documents` function
...
- **Vector Store**: `vector_store/README.md` - Supabase pgvector operations
...
**Status**: 🎉 **100% Core Features Complete** - Full RAG pipeline operational with Hugging Face Mistral-7B, Supabase pgvector retrieval, and production-ready API endpoints (`POST /rag/query`, `GET /rag/health`).
