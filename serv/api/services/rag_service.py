    ...
    async def initialize(self):
        ...
        try:
            ...
            if settings.use_local_embeddings:
                logger.warning("[RAG] WARNING: Using local embeddings. Ensure n8n is also using this model or retrieval will fail!")
            ...
            logger.info("[RAG] Connecting to Supabase vector store...")
            if not settings.supabase_url or not settings.supabase_key:
                logger.warning("[RAG] SUPABASE_URL or SUPABASE_KEY not set. Vector store disabled.")
            
            self.vector_store = SupabaseVectorStore(
                url=settings.supabase_url,
                key=settings.supabase_key,
                collection_name=settings.supabase_collection_name,
...
To enable AI-powered coaching, please ensure your document ingestion pipeline (n8n) is running and has processed some documents.

Once documents are available in Supabase, I'll be able to provide personalized productivity tips, focus strategies, and study guidance!
